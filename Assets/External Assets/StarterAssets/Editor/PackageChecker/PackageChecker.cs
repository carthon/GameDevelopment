...
namespace StarterAssets {
    public static class PackageChecker {
        ...
        private static void CheckPackage() {
            ...
            if (!ScriptingDefineUtils.CheckScriptingDefine(PackageCheckerScriptingDefine)) {
                ...
                if (requiredPackagesListFile.Length == 0) {
                    ...
                    var packageListPath = requiredPackagesListFile[0];
                    ...
                    for (var i = 0; i < installRequired.Length; i++) installRequired[i] = false;

                    // build data collections compatible packages for this project, and packages within the project
                    var compatiblePackages =
                        new List<PackageInfo>();
                    var clientPackages =
                        new List<PackageInfo>();

                    foreach (var result in compatibleList.Result) compatiblePackages.Add(result);

                    foreach (var result in clientList.Result) clientPackages.Add(result);

                    // check for the latest verified package version for each package that is missing a version
                    for (var i = 0; i < packagesToAdd.Count; i++) {
        ...
        public static int CompareVersion(string latestVerifiedVersion, string projectVersion) {
            var latestVersionSplit = latestVerifiedVersion.Split('.');
            var projectVersionSplit = projectVersion.Split('.');
            var iteratorA = 0;
            var iteratorB = 0;

            while (iteratorA < latestVersionSplit.Length || iteratorB < projectVersionSplit.Length) {
                var latestVerified = 0;
                var installed = 0;

                if (iteratorA < latestVersionSplit.Length) latestVerified = Convert.ToInt32(latestVersionSplit[iteratorA]);

                if (iteratorB < projectVersionSplit.Length) installed = Convert.ToInt32(projectVersionSplit[iteratorB]);
